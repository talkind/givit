...
class Profile(models.Model):
    ...
    phone_regex = RegexValidator(
        regex=r'^05[0-68-9][0-9]{7}$', message="Phone number must be entered in the format: 05XXXXXXXX.")
    id_regex = RegexValidator(
        regex=r'[0-9]{8,9}', message="ID must be 9 digits long.")
    house_number_regex = RegexValidator(
        regex=r'[0-9]{1-3}', message="Please enter a valid number from 1 to 999.")
    ...
    phone_number = models.CharField(
        help_text='example: 0544544554', max_length=10, validators=[phone_regex])
    ...
    address_house_number = models.CharField(
        max_length=3, verbose_name='home_number', validators=[house_number_regex])
